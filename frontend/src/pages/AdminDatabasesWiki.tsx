...
export const AdminDatabasesWiki: React.FC = () => {
    ...
    const loadDatabases = async () => {
        try {
            const data = await databaseService.getAllWiki();
            console.log('Wiki Databases data:', data);
            ...
            console.error('Load wiki databases error:', error);
            toast.error('Ошибка загрузки баз данных Wiki');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const loadFilters = async () => {
        try {
            const data = await filterService.getAllWiki();
            console.log('Loaded Wiki filters:', data);
            setFilters(data || []);
        } catch (error: any) {
            console.error('Load Wiki filters error:', error);
        }
    };
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            await databaseService.uploadWiki(formData);
                                            ...
                                            >
                                                {columns.map(col => (
                                                    <td key={col.key} className="p-3 text-sm">
                                                        {col.render ? col.render(db) : (db as any)[col.key]}
                                                    </td>
                                                ))}
                                            </tr>
                                        ))
